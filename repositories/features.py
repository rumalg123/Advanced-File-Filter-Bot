...
class FeatureRepository:
    ...
    async def create_file_report(
        self, user_id: int, file_unique_id: str, reason: str
    ) -> tuple[dict[str, Any], bool]:
        collection = await self._collection("file_reports")
        report_id = hashlib.sha256(
            f"{user_id}:{file_unique_id}:{reason}".encode()
        ).hexdigest()[:16]
        now = datetime.now(UTC)
        document = {
            '_id': report_id,
    ...
    async def resolve_file_report(self, report_id: str, admin_id: int) -> bool:
        collection = await self._collection("file_reports")
        result = await self.db_pool.execute_with_retry(
            collection.update_one,
            {'_id': report_id, 'status': 'open'},
            {
                '$set': {
                    'status': 'resolved',
